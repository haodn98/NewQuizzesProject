...
async def test_quiz(test_user, test_company_with_member):
...
@pytest.fixture(scope="function")
async def test_result_to_test_quiz(test_quiz, test_company_with_member, test_user):
...
async def test_get_all_quizzes(ac: AsyncClient, test_user, test_quiz):
...
async def test_get_all_quizzes_paginated(ac: AsyncClient, test_user, test_quiz):
...
async def test_get_quiz(ac: AsyncClient, test_user, test_quiz, test_company_with_member):
...
async def test_get_quiz_wrong_id_format(ac: AsyncClient, test_company_with_member):
...
async def test_get_quiz_wrong_quiz_id(ac: AsyncClient, test_company_with_member):
...
async def test_get_quiz_wrong_company_id(ac: AsyncClient, test_quiz):
...
async def test_get_quiz_with_answers(ac: AsyncClient, test_user, test_quiz, test_company_with_member):
...
async def test_get_quiz_with_answers_wrong_id_format(ac: AsyncClient, test_company_with_member):
    response = await ac.get("/quizzes/{}/{}".format(test_company_with_member.id, "1000"))
...
async def test_get_quiz_with_answers_wrong_quiz_id(ac: AsyncClient, test_company_with_member):
    response = await ac.get("/quizzes/{}/{}".format(test_company_with_member.id, "222222222222222222222222"))
...
async def test_get_user_average_with_wrong_company(ac: AsyncClient,
    ...
    response = await ac.get("/quizzes/average_mark?max_day=2023-09-29&company_id=3")
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert response.json() == {"detail": "No quiz results for this period of time or with this company"}
